...
/*예매 버튼이 눌릴 때 예매번호 생성해서 폼에 같이 넘기기*/
	...
	$(".headerorderbtn").click(function(){
		var oid = "";
		$.ajax({
			url:"http://localhost:9000/gga_spring1/oidProc.do",
			success:function(result){
				oid = result;
				location.replace("http://localhost:9000/gga_spring1/order.do?oid="+oid);
			}
	});
	});
	...
	$(".headerorderbtn1").click(function(){
		...
		$.ajax({
			...
			success:function(result){
				oid = result;
				location.replace("http://localhost:9000/gga_spring1/order.do?oid="+oid+"&movieid="+movieid);
			}
	});
	});
	...
	$("#seatcardbtn").click(function(){
		...
		if(seatcom == ""){
			...
			IMP.request_pay({
	            ...
	            /* m_redirect_url: "http://localhost:9000/gga_spring1/ordercon.do"  */
            }, function (rsp) { // callback
                if (rsp.success) {
                	  $.ajax({
          				url:"http://localhost:9000/gga_spring1/orderconProc.do?seat="+seatcom+"&price="+seattotal+"&oid="+oid,
          				success: function(result){
          					if(result==1){
          						location.href="http://localhost:9000/gga_spring1/ordercon.do";
          					}else if(result==0){
	           ...
	           /* m_redirect_url: "http://localhost:9000/gga_spring1/ordercon.do" */
                ...
                if (rsp.success) {
                    ...
                    jQuery.ajax({
                        ...
                        // 성공시 로직
                    		location.replace("http://localhost:9000/gga_spring1/ordercon.do?oid="+oid);
                    	})
                    } else {
                    console.log(rsp);
                    alert("결제를 실패했습니다. 잠시후 다시 시도해 주세요.");
	...
	$("#btnSearch").click(function(){
		if($("#mtitle").val() =="" ){
			...
			$.ajax({
		          url:"http://localhost:9000/gga_spring1/searchproc.do?mtitle="+$("#mtitle").val(),
		          success: function(movieid){
		          	if(movieid == ""){
						alert("상영 예정 없는 영화입니다 다른검색어를 입력해주세요");
		          	}else{
		           	 location.href="http://localhost:9000/gga_spring1/movieinfo.do?movieid="+movieid; }
		         	}
	...
	$(".mimg").click(function(){
		...
		$(location).attr("href", "http://localhost:9000/gga_spring1/movieinfo.do?movieid="+movieid)
	});
	/**********************************
...
// -- 오장원
	...
	$("#btnIdCheck").click(function(){
		if ($("#id").val() == ""){
			...
			$.ajax({
				url : "http://localhost:9000/gga_spring1/idCheckProc.do?id=" + $("#id").val(),
				success : function(result){
					if (result == 1){
